...
static Status lpc_mii_write(uint8_t reg, uint16_t data, uint32_t phyAddr)
{
	Status sts = ERROR;
	int32_t mst = 250;

	/* Write value for register */
	Chip_ENET_StartMIIWrite(LPC_ETHERNET, reg, data, phyAddr);
...
/* Read from the PHY. Will block for delays based on the pDelayMs function. Returns
   true on success, or false on failure */
static Status lpc_mii_read(uint8_t reg, uint16_t *data, uint32_t phyAddr)
{
	Status sts = ERROR;
	int32_t mst = 250;

	/* Start register read */
	Chip_ENET_StartMIIRead(LPC_ETHERNET, reg, phyAddr);
...
uint32_t lpc_phy_init(bool rmii, p_msDelay_func_t pDelayMsFunc, uint32_t phyAddr)
{
	uint16_t tmp;
	int32_t i;
	...
	if (lpc_mii_write(LAN8_BCR_REG, LAN8_RESET, phyAddr) != SUCCESS) {
		return ERROR;
	}
	i = 400;
	while (i > 0) {
		pDelayMs(1);
		if (lpc_mii_read(LAN8_BCR_REG, &tmp, phyAddr) != SUCCESS) {
			return ERROR;
		}
	...
	/* Setup link */
	lpc_mii_write(LAN8_BCR_REG, LAN8_AUTONEG, phyAddr);
...
uint32_t lpcPHYStsPoll(uint32_t phyAddr)
{
	static uint16_t sts;
	...
	case 0:
		...
		Chip_ENET_StartMIIRead(LPC_ETHERNET, LAN8_BSR_REG, phyAddr);
		physts &= ~PHY_LINK_CHANGED;
	...
	case 1:
		...
		if (!Chip_ENET_IsMIIBusy(LPC_ETHERNET)) {
			...
			Chip_ENET_StartMIIRead(LPC_ETHERNET, LAN8_PHYSPLCTL_REG, phyAddr);
			phyustate = 2;
		}
		break;
